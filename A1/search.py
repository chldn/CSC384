'''Search routines.
   A) Class StateSpace
      ...
      - abstract base class 
      - represents search space states 
      - each state has pointer to the parent used to generate it
      - g-value cost of action sequence used to generate it

      Equivalent states can be reached via different paths
        Search routines employ cycle checking using hashing techniques
            to avoid exploring the same state multiple times
        
      Hence, each StateSpace state (or object) 
        returns an immutable representation that uniquely represents the state 
                                                & used to index into a dictionary

      The StateSpace class specialized for the particular problem.
        - each problem defines a subclass of StateSpace that will also
      include information specific to that problem. 
      
      See WaterJugs.py for an example, and the Class implementation for more details.


    B) class SearchEngine
        - main routines employed are here:
            - set search stratgey
            - invoke search (init_search)
            - resue search afer goal found (searchOpen)
      - defines search routines
      Utilizes 2 auxiliary classes:
        (1) sNode---class objects used to represent nodes in search space 
                    - contain problem states 
                                - StateSpace objects that are search nodes (not in state space)
        (2) Open---used to store unexpanded nodes set 
                    - search strategy specific. 
            For example, Open is implemented as a stack when doing depth-first search, 
                                            as a priority queue when doing astar search etc.
    '''
import heapq
...
class StateSpace:
    ...
    def __init__(self, action, gval, parent):
        '''Problem specific state space objects must always include the data items
           a) self.action === action name used to generate this state from parent
                            if initial state : action name "START"
           b) self.gval === int/real number - cost of getting to this state
           c) parent - state from which this state was generated by applying "action"
        '''
        self.action = action
        self.gval = gval
        self.parent = parent
    ...
    def successors(self):
        '''Returns list of successor states, each with the data items:
            - "action" - action used to generate this successor state
            - "gval" cost of self + cost of the action
            - parent set to self.
           Also any problem specific data must be specified property.'''        
        raise Exception("Must be overridden in subclass.")

    def hashable_state(self):
        '''Returns an immutable & unique representation of state represented by self. 
        The return value (str/tuple) will be used by hashing routines. 
        
        If obj1 & obj2 are StateSpace objects:
            if obj1 and obj2 represent same problem state:
                obj1.hashable_state() == obj2.hashable_state() 
        '''
        raise Exception("Must be overridden in subclass.")
...
class sNode:
    '''Object of this class form the nodes of the search space.  
    
    Each node consists of:
        - search space object (determined by problem definition)
        - h value 
        - g value - redundant as it is stored in the state, but we make a copy in the
...
class Open:
    '''Open - hold search frontier
                        - set of unexpanded nodes
       Depending on search strategy used,
            extract nodes from this set in different orders
        so set up object's functions to operate as needed by particular search strategy'''
